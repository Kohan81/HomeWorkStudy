package org.stepic.level_3.mailService;
...
public class MainClassMailService {
...
*/
    ...
    */
    ...
    public static class Inspector implements MailService {
        ...
        public Sendable processMail(Sendable mail) {
            if (mail instanceof MailPackage) {
                ...
                } if (content.startsWith("stones instead of ")){
    ...
    public static class UntrustworthyMailWorker implements MailService {
        ...
        private MailService[] someAddressee;
        private RealMailService realAddressee;

        public UntrustworthyMailWorker (MailService[] addressee){
            this.someAddressee = addressee;
            realAddressee = new RealMailService();
        }

        public MailService getRealMailService() {
            return realAddressee;
        }
        ...
        public Sendable processMail(Sendable mail) {
            ...
            for (MailService addressee : someAddressee) {
                sendable = addressee.processMail(sendable);
            }
            return realAddressee.processMail(sendable);
        }
    }
 ...
 *
 * 1) UntrustworthyMailWorker – класс, моделирующий ненадежного работника почты, который вместо того, чтобы
 * передать почтовый объект непосредственно в сервис почты, последовательно передает этот объект набору третьих лиц,
 ...
 * конструируется от экземпляра Logger, с помощью которого шпион будет сообщать о всех действиях. Он следит только за
 * объектами класса MailMessage и пишет в логгер следующие сообщения (в выражениях нужно заменить части в фигурных
